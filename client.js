...
client.connect(port, host, function() {
    ...
    readline.question(
		...
		to join a private room write "gameCode-number" `, (word) => {
        client.write(word);
    })
});
...
client.on('data', async function(data) {
    ...
    if(message.type === "gameStart") {
        ...
        if(yourTurn) {
            ...
            readline.question('which box ? ', (boxNumber) => {
                ...
                client.write(JSON.stringify({type:'playedMove', data: playedMove}));     
            })
        }
    }
    ...
    if (message.type === 'otherTurn') {
		if (checkWinner()){
			...
			if (checkTie()){
				client.write(JSON.stringify({type: "tie", data: playerData.roomId}));
			}else{
				yourTurn = false
                showGameState();
				checkTurn()
